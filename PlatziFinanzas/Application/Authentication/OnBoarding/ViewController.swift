...
class OnBoardingViewController: UIPageViewController {
    var pageControl: UIPageControl! // ! significa que se que existe
    
    // es una propiedad que solo se puede acceder desde este archivo (2)
    fileprivate(set) lazy var items: [OnBordingItem] = {
        return [
            OnBordingItem(
    ...
    func instanceViewController(_ index: Int ) -> UIViewController {
        ...
        guard let viewController =  UIStoryboard(name: "OnBoarding", bundle: Bundle.main )
            ...
            as? OnBordingStepsViewController else { // del tipo  y sino
